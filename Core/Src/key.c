  ...
  key_t.read = _KEY_ALL_OFF; //0xFF 
    ...
    switch(key_t.state )
	{
		...
		case second:
		{
			...
			{
				if(++key_t.on_time>10000 && tpd_t.keep_heat_fun_digital_numbers==1)// 500 long key be down
				{
				...
				{
					if(++key_t.off_time>5) //8 //30 don't holding key dithering
					{
